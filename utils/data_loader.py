...
class DataLoaderFactory:
    ...
    def get_data_loaders(self):
        if self.train_dataset == 'cifar10':
            ...
            trainset, validset, testset = self.get_cifar100_loaders()
        elif self.train_dataset == 'augcifar10':
            trainset, validset, testset = self.get_aug_cifar10_loaders()
        elif self.train_dataset == 'augcifar100':
            trainset, validset, testset = self.get_aug_cifar100_loaders()
        ...
        valid_loader = DataLoader(validset, batch_size=self.batch_size, sampler=valid_sampler,
                                  num_workers=self.num_workers, pin_memory=self.pin_memory)
    ...
    def get_attack_loader(self, eval_noise):
        ...
        if self.eval_dataset == 'cifar100C':
            return self.get_cifar100c_attack_loader(eval_noise)
        if self.eval_dataset == 'augcifar10C':
            return self.get_cifar10c_attack_loader(eval_noise)
        if self.eval_dataset == 'augcifar100C':
            return self.get_cifar100c_attack_loader(eval_noise)
    ...
    def get_cifar10c_attack_loader(self, eval_noise):
        ...
        cifar10c_attack_loader = DataLoader(
            cifar10c_dataset, batch_size=self.batch_size, shuffle=False)

        return cifar10c_attack_loader

    def get_cifar100c_attack_loader(self, eval_noise):
    ...
    def get_aug_cifar100_loaders(self):
        # Define transforms
        transform_train = transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(),
    ...
    def get_imagenetC_attack_loader(self, eval_noise, n_classes=10):
        ...
        eval_noise = eval_noise[:-
                                4] if eval_noise.endswith('.npy') else eval_noise

        testset = ImageNetKaggle(os.path.join(self.root, "imagenetc"), 'eval',
                                 transform=transform_imagenetc, noise=eval_noise, n_classes=n_classes)
        data_loader = DataLoader(testset, batch_size=self.batch_size, shuffle=False,
                                 num_workers=self.num_workers, pin_memory=self.pin_memory)
    ...
    def get_imagenet_loaders(self, n_classes=10):
        ...
        trainset = ImageNetKaggle(os.path.join(
            self.root, "imagenet"), 'train', transform=transform_train, n_classes=n_classes)
        validset = ImageNetKaggle(os.path.join(
            self.root, "imagenet"), 'train', transform=transform_train, n_classes=n_classes)
        testset = ImageNetKaggle(os.path.join(
            self.root, "imagenet"), 'val', transform=transform_test, n_classes=n_classes)
...
if __name__ == "__main__":
    ...
    factory = DataLoaderFactory(
        root='./data', valid_size=0.1, train_dataset='imagenet', eval_dataset='imagenetC')

    train_loader, valid_loader, test_loader = factory.get_data_loaders()
    imagenetc_attack_loader = factory.get_attack_loader(
        eval_noise='defocus.npy')

    images_train, labels_train = next(iter(train_loader))
    plot_images_grid(images_train, labels_train, nrow=2,
                     ncol=2, path="results/plots/trainclean.png")

    images_valid, labels_valid = next(iter(valid_loader))
    plot_images_grid(images_valid, labels_valid, nrow=2,
                     ncol=2, path="results/plots/validclean.png")

    images_test, labels_test = next(iter(test_loader))
    plot_images_grid(images_test, labels_test, nrow=2, ncol=2,
                     path="results/plots/testclean.png")

    images_attack, labels_attack = next(iter(imagenetc_attack_loader))
    plot_images_grid(images_attack, labels_attack, nrow=2,
                     ncol=2, path="results/plots/cleanc.png")
