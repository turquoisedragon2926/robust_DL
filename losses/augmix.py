...
def data_aug(image, preprocess, mixture_width=np.random.randint(1, 5), mixture_depth=np.random.randint(1, 4), aug_severity=np.random.randint(1, 5)):
    """Perform AugMix augmentations and compute mixture.
...
def single_image_aug(image, preprocess, mixture_width=np.random.randint(1, 5), mixture_depth=np.random.randint(1, 4), aug_severity=np.random.randint(1, 5)):
    ...
    for i in range(mixture_width):
        ...
        image_aug = pil_to_tensor(image_aug).to(device) if isinstance(image_aug, Image.Image) else image_aug
        mix += ws[i] * preprocess(image_aug)
